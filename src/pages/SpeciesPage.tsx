...
import { SW_SpeciesSearchResponse } from '../types'
...
const SpeciesPage = () => {
    ...
    const [searchResult, setSearchResult] = useState<SW_SpeciesSearchResponse | null>(null)
    ...
    const getSpecies = async (searchPage: number) => {
        setLoading(true)
        setShowErr(false)
        setError(null)

        try {
            const data = await SW_API.getSpecies(searchPage)
    ...
    const searchSpecies = async (searchQuery: string, searchPage: number) => {
        setError(null)
        setShowErr(false)
        setLoading(true)
        ...
        try {
            const data = await SW_API.searchSpecies(searchQuery, searchPage)
        ...
        <div className="species">
            ...
            <Container className="py-3">
                <div className="bg-card py-4 px-4">
                    ...
                    {searchResult && (query ? <p>Showing {searchResult.total} search results for "{query}"</p> : <p> {searchResult.total} species</p>)}
                </div>

                {searchResult && (
                    <div id="species" className="py-3">
                                ...
                                <Col key={data.id} className="d-flex align-items-stretch">
                                    <Card className="glass">
                                        <Card.Body className="d-flex row px-4">
                                            ...
                                            <Card.Text>
                                                <strong>Characters:</strong> {data.people_count}
                                            </Card.Text>
                                            <Card.Text>
